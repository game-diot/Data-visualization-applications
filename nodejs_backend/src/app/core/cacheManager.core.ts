...
export const cacheManager = {
  ...
  async connect() {
    try {
      ...
      console.error("❌ Redis 连接失败，应用程序将退出。");
      process.exit(1);
    }
  ...
  async set(key: string, value: string, ttl?: number) {
    ...
    if (!redisClient.isReady) {
      console.warn(`⚠️ Redis 客户端未就绪，SET 操作 (${key}) 被跳过。`);
      return null;
    }
  ...
  async get(key: string) {
    if (!redisClient.isReady) {
      console.warn(`⚠️ Redis 客户端未就绪，GET 操作 (${key}) 返回 null。`);
      return null;
    }
  ...
  async del(key: string) {
    if (!redisClient.isReady) {
      console.warn(`⚠️ Redis 客户端未就绪，DEL 操作 (${key}) 返回 0。`);
      return 0;
    }
